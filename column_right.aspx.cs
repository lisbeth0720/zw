...
{
    public partial class column_right : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (mmodel.IsOnLine == 1)
            {
                ...
                {
                    column_right_type.Value = Request["type"];
                }
                if (!string.IsNullOrEmpty(Request["id"]) && !string.IsNullOrEmpty(Request["idtype"]))
                {
                    ...
                    if (cmService.IsShouQuan(mmodel, 0, CommonRight.RIGHT_C_Auth) || authid == mmodel.Userid)
                    {
                        ...
                        string selright = "";
                        DataTable aTable = aService.GetAllRightByAuthID(mmodel.Companyid, authid, 1, idtype);
                        ...
                        if (idtype == 0)
                        {
                            ...
                            if (manager.Quickcheck.Split(';').Length > 1)
                            {
                                selright += manager.Quickcheck.Split(';')[1] + ",";
                            }
                            Depart depart = dService.GetDepartByID(mmodel.Companyid, Convert.ToInt32(manager.DeptID));

                            if (depart.QuickCheck.Split(';').Length > 1)
                            {
                                selright += "p_" + depart.QuickCheck.Split(';')[1] + ",";
                            }

                            DataTable pTable = aService.GetAllRightByAuthID(manager.Companyid, Convert.ToInt32(manager.DeptID), 1, 1);
                        ...
                        else
                        {
                            ...
                            selright += depart.QuickCheck.Split(';')[1];
                        }
                        column_right_hid_seldep.Value = selright;
                    }
                    else
                    {
