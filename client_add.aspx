...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $(".titleContent h6").click(function () {
            var index = $(this).index();
            $(".titleContent h6").removeClass("actyiveTitleContent");
            $(this).addClass("actyiveTitleContent")
            //// $("#client_add_ex" + index).click();
            $(".section").hide();
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(json.Table, function (idx, item) {
                        ...
                        $("#client_add_password").val(item.password);
                        if (item.picture == "") {
                        ...
                        var clientHost = item.host;
                        $("#client_add_host").val(clientHost);
                        ...
                        if (item.startuptime.length >= 5) {
                            ...
                            //分号为关键字，系统中使用~符号代替分号，取值时再转换回来
                            startupexcept = item.startuptime.charAt(5);
                            startupexcept = startupexcept.replace("~", ";");
                            if (item.startuptime.length == 6) {

                                if (item.startuptime.substring(5, 1) != "_") {
                                    $("#client_add_startupexcept").val(startupexcept.charCodeAt() - 48);
                                }
                                else {
                            ...
                            if (item.startuptime.length == 1) {
                                startupexcept = item.startuptime.charAt(0);
                                startupexcept = startupexcept.replace("~", ";");
                                $("#client_add_startupexcept").val(startupexcept.charCodeAt() - 48);
                            }
        ...
        //$("#client_add_picture_upload").on("change", function () {
        ...
        $("#client_add").Validform({
            ...
            beforeSubmit: function (curform) {
                ...
                //var res = checkName();
                //if (res != "") {
                //    $("#ipNull").html("*" + res).show();
                //    TopTrip("终端网址/MAC地址：已经存在", 2);
                //    return false;
                //} else {
                //    $("#ipNull").hide();
                //}
            ...
            callback: function (d) {
                ...
                if (d.status == "y") {
                    ...
                    if ($("#client_add_isedit").val() == "") {
                        ...
                        ////设置开关机时间 //发指令
                        // SetShutDownTimes($("#client_add_groupid").val());//设置单个终端
                        //批量修改呢//组内所有终端？？idlist
                    }
                    //loadParentPage();//
        ...
        if ($(ele).val() != "") {
            $.ajaxFileUpload({
                ...
                success: function (data, status) {
                    ...
                    if (data <= 0) {
                        showError("client_add_picture", getLanguageMsg("上传错误", $.cookie("yuyan")), 0);
                    }
                    else {
    ...
    function checkName() {
        var result = "";
        $.ajax({
            url: 'ajax/CheckClientName.ashx',
            type: 'post',
            dataType: 'json',
            async: false,
            data: { 'ip': $("#client_add_ip").val(), 'mac': $("#client_add_mac").val(), 'clientid': $("#client_add_idlist").val() },
            success: function (data) {
                //debugger;
                if (data.status == "y") {
                    //return "";
    ...
    $("#client_fanhui").click(function () {
        if ($.cookie("slect_tab") == "终端管理") {
            if ($.cookie("mySelectNodeID") == null || $.cookie("mySelectNodeID") == "") {
                $("#rootClient").click();
            } else {
                $("#" + $.cookie("mySelectNodeID").split("_")[0] + "_" + $.cookie("mySelectNodeID").split("_")[1] + "_span").click();
            }
        } else {
            client_main_loadright(0, clientid, dlevel, clientmark);
        }
...
<form id="client_add" action="ajax/saveclient.ashx" method="post" style="overflow: hidden;">
    ...
    <div id="baidumapdiv" style="position: fixed; top: 100px; width: 820px; height: 520px; left: 620px; z-index: 9999; display: none;">
        <iframe src="/company/source/BaiduMap.aspx" width="820" height="520" frameborder="0" style="padding: 0;"></iframe>
        <%--id="clientbaidumapdiv"，BaiDuMap.html--%>
    </div>
    <div class="titleContent">
        <h6 class="actyiveTitleContent">基本属性</h6>

        <h6>扩展属性</h6>
        <h6>区域属性</h6>
    </div>
    <div class="zd_add_box" id="client_addbox" style="width: 978px; border: none; padding: 0;">
        <%--padding为0是去掉上面10px的白色距离--%>
        <%-- <h6>播放终端<tt class="Validform_checktip">(*为必填字段)</tt></h6>--%>
        <div class="section sec_box">
            <ul class="clearfix">
                <li>
                    <span class="label language">终端名称：</span>
                    <input class="inp_t" id="client_add_name" name="client_add_name" datatype="*" errormsg="终端名称不能为空" type="text" />
                    <tt class="Validform_checktip language">*唯一标记，最长64字节。</tt>

                </li>
                <li style="display: none"><span class="label language">终端内部码：</span><input class="inp_t" id="client_add_mark" readonly="readonly" name="client_add_mark" runat="server"></li>
                ...
                <li><span class="label">序列号验证：</span><input class="inp_t" id="client_add_password" maxlength="20" name="client_add_password" placeholder="aaaaaa" type="text"></li>
                <li><span class="label language">播放终端端口号：</span><input class="inp_t" style="width: 100px;" id="client_add_port" name="client_add_port" type="text">
                    <span class="label language" style="width: 120px;">节目单分发顺序：</span><input class="inp_t" style="width: 100px;" id="client_add_postion" name="client_add_postion" type="text" value="0"><span class="label" style="width: 132px;">(数字越小，优先级越高)</span></li>
                <li><span class="label language">描述（最多1024汉字）：</span><div class="intro" style="width: 490px;">
                    <textarea class="txtarea" id="client_add_descript" name="client_add_descript" style="width: 447px;"></textarea>
                </div>
                ...
                <li><span class="label language">每天开机时间：</span><input class="inp_t" style="width: 100px;" onclick="WdatePicker({ dateFmt: 'HH:mm' })" id="client_add_startuptime" name="client_add_startuptime" type="text" autocomplete="off">
                    <span class="prompt">
                        <select name="client_add_startupexcept" id="client_add_startupexcept" class="inp_t">
                            <%--style="width:170px;"--%>
                ...
                <li><span class="label language">每天关机时间：</span><input class="inp_t" style="width: 100px;" onclick="WdatePicker({ dateFmt: 'HH:mm' })" id="client_add_shutdowntime" name="client_add_shutdowntime" type="text" autocomplete="off">
                    <span class="prompt">
                        <select name="client_add_waitacktime" class="inp_t" id="client_add_waitacktime">
                        ...
                        </select></span><input type="hidden" name="client_add_waitacktime1" id="client_add_waitacktime1" value="0" /></li>
            </ul>
        </div>
        <h6 class="language" style="display: none;">扩展属性</h6>
        ...
        <div class="section" style="display: none; padding-bottom: 0;">
            <ul class="clearfix">
                ...
                <li><span class="label language">控制服务器端口号：</span><input class="inp_t" id="client_add_ctlport" name="client_add_ctlport" type="text"><span class="label" style="text-align: left; margin-left: 15px;"><input type="checkbox" name="client_add_ctloption" value="4" id="ctloptioncc" /><label for="ctloptioncc" class="language">启用控制服务器</label></span></li>
                ...
                <li><span class="label language">远程控制颜色数：</span><p style="width: 460px; float: left;">
                    ...
                    <span class="label" style="width: 75px; text-align: left">
                        <input name="client_add_ctlcolor" value="1" type="radio" id="ctlcolor_bb"><label for="ctlcolor_bb" class="language">16位色</label></span>
        ...
        <h6 class="language" style="display: none">区域属性</h6>
        ...
        <div class="section" style="display: none;">
            <ul class="clearfix">
                ...
                <li>
                    ...
                    <input type="file" name="client_add_picture_upload" accept="image/jpg,image/png,image/gif,image/bmp,image/jpeg" style="float: left; height: 25px; width: 228px;" id="client_add_picture_upload" onchange="clientImage(this)" />
                ...
                <li><span class="label language">地理坐标(经度)：</span><input class="inp_t" style="width: 100px;" id="sc_add_coordx" name="sc_add_coordx" value="0.0" type="text" readonly="readonly">
                    <!--margin-left:54px;-->
                    <span class="label language" style="margin-left: 30px;">地理坐标(纬度)：</span><input class="inp_t" style="width: 112px;" id="sc_add_coordy" name="sc_add_coordy" value="0.0" type="text" readonly="readonly"><span class="prompt prompt_pic"><img src="/images/baidumap.png" alt="定位" onclick="client_loadBaiduMap()" style="cursor: pointer" /></span></li>
        ...
        <div class="sc_add_btn clearfix" style="margin-left: 190px; float: left;">
            <span class="inp_btn addbtn pro_genxin " style="position: relative;">
                <b style="left: 20px;"></b>
            ...
            <span class="inp_btn fanhui" style="position: relative;">
                ...
                <input class="btn language" value="返回" id="client_fanhui" type="button" style="padding-left: 25px;"></span>
        </div>
    </div>
</form>
