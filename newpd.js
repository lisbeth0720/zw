  ...
  // $(".otherFunctionContent").css("width", ($("#otherFunction").width() - $(".otherFunctionBtn").width()) + "px");
    ...
    setTimeout(function(){
        $("#bar4 .keys").click(function () {
            var keyCode="";
            if($(".keysCommon").attr("mackUp")=="mackUp"){
                //A-Z a-z发送指令时不加ff
                if($(this).attr("shift-data-id")!=undefined){
                    keyCode = ("0X" + (parseInt($(this).attr("shift-data-id")).toString(16).length > 1 ? parseInt($(this).attr("shift-data-id")).toString(16) : "0" + parseInt($(this).attr("shift-data-id")).toString(16))).toUpperCase();
                }else{
                    keyCode = ("0X" + (parseInt($(this).attr("data-kid")).toString(16).length > 1 ? parseInt($(this).attr("data-kid")).toString(16) : "0" + parseInt($(this).attr("data-kid")).toString(16))).toUpperCase();
                }
            }else{
               keyCode = ("0X" + (parseInt($(this).attr("data-kid")).toString(16).length > 1 ? parseInt($(this).attr("data-kid")).toString(16) : "0" + parseInt($(this).attr("data-kid")).toString(16))).toUpperCase();
            }
        ...
        $("#bar4 .keysCmd ").click(function () {
            ...
            if($(".keysCommon").attr("mackUp")=="mackUp"){
                if($(this).attr("shift-data-id")!=undefined){
                    keyCode = ("0XFF" + (parseInt($(this).attr("shift-data-id")).toString(16).length > 1 ? parseInt($(this).attr("shift-data-id")).toString(16) : "0" + parseInt($(this).attr("shift-data-id")).toString(16))).toUpperCase(); 
                }else{
                    keyCode = ("0XFF" + (parseInt($(this).attr("data-kid")).toString(16).length > 1 ? parseInt($(this).attr("data-kid")).toString(16) : "0" + parseInt($(this).attr("data-kid")).toString(16))).toUpperCase();
                }
            }else{
               keyCode = ("0XFF" + (parseInt($(this).attr("data-kid")).toString(16).length > 1 ? parseInt($(this).attr("data-kid")).toString(16) : "0" + parseInt($(this).attr("data-kid")).toString(16))).toUpperCase();
            }
             if($(this).attr("changeCase")!=undefined&&$(this).attr("changeCase")=="lowerCase"){
                
                for(var i=0;i<$(".data-letter").length;i++){
                    $(".data-letter").eq(i).html($(".data-letter").eq(i).html().toUpperCase())
                }
                $(this).attr("changeCase","upperCase");
             }else if($(this).attr("changeCase")!=undefined&&$(this).attr("changeCase")=="upperCase"){
                for(var i=0;i<$(".data-letter").length;i++){
                    $(".data-letter").eq(i).html($(".data-letter").eq(i).html().toLowerCase())
                }
                $(this).attr("changeCase","lowerCase");
             }
            docmd('keycode', keyCode);
        })
        ...
        $(".keysCommon").click(function(){
            if($(".keysCommon").attr("mackUp")=="mackUp"){
                $(".keysCommon").css("background","#333");
                $(".keysCommon").attr("mackUp","nomackUp");
            }else{
                $(".keysCommon").css("background","#dedede");
                $(".keysCommon").attr("mackUp","mackUp");
            }   
        })
    },2000)
	...
	$(".topLogin img").click(function(){
		$(".onClickLogin").hide();
		$(".onClickLogin input[type=text]").val("");
    })
...
window.onload = function () {
    ...
    $("#top").attr("color", $("#top li").eq(clientIndex).attr("color"));
    $("#top").attr("onlyview", $("#top li").eq(clientIndex).attr("onlyview"));
    $("#top").attr("usegate", $("#top li").eq(clientIndex).attr("usegate"));
    $("#top").attr("gateip", $("#top li").eq(clientIndex).attr("gateip"));
    $("#top").attr("gateport", $("#top li").eq(clientIndex).attr("gateport"));
    $("#top").attr("port", $("#top li").eq(clientIndex).attr("port"));
    color = $("#top li").eq(clientIndex).attr("color");//颜色值
    onlyview = $("#top li").eq(clientIndex).attr("onlyview");//是否是只允许预览
    usegate = $("#top li").eq(clientIndex).attr("usegate");//是否是多个服务器进行转发指令
    gateip = $("#top li").eq(clientIndex).attr("gateip");//进行转发的多个服务器的ip地址，以;分割
    gateport = $("#top li").eq(clientIndex).attr("gateport");//目的服务器的显示端的端口号
    port = $("#top li").eq(clientIndex).attr("port");//目的服务器的显示端的端口号
    ...
    $(".clientName").click(function () {
        ...
        $("#top").attr("color", $(this).parent().attr("color"));
        $("#top").attr("onlyview", $(this).parent().attr("onlyview"));
        $("#top").attr("usegate", $(this).parent().attr("usegate"));
        $("#top").attr("gateip", $(this).parent().attr("gateip"));
        $("#top").attr("gateport", $(this).parent().attr("gateport"));
        color = $(this).parent().attr("color");//颜色值
        onlyview = $(this).parent().attr("onlyview");//是否是只允许预览
        usegate = $(this).parent().attr("usegate");//是否是多个服务器进行转发指令
        gateip = $(this).parent().attr("gateip");//进行转发的多个服务器的ip地址，以;分割
        gateport = $(this).parent().attr("gateport");//目的服务器的显示端的端口号
        newurl = $("#top").attr("src");
           ...
           $("#functionButton").css("display", "block");
            if ($(xml).find("item").length <= 0) {
                ...
                $(xml).find("item").each(function (i) {
                    ...
                    var inImage = "<div class='startNow'><img  src='images/liebiao/bofang.png' class='playNowprogram' onclick=\"startNow(this,'" + taskID + "');\"/><img class='moreProgramInfo' src='images/liebiao/gengduo.png' /></div>";//'"docmd(16,"'+taskID+'")"'
                    ...
                    var columnInfoImage ="<div class='startNow'><img class='moreProgramInfo' src='images/contentTab/gengduoex.png' style='float:right;'/></div>";
                    ...
                    if ($(this).find("taskName").text().indexOf("**") >= 0) {
                        ...
                        if($(this).find("itemType").text().indexOf("14")>=0|| $(this).find("itemType").text().indexOf("1014")>=0){
                            content1 += "<div class='left' taskid='" + taskID + "' columId=" + $(this).find("markID").text().split("-")[2]+">" + imagesrc + "<div class='leftList' iconLink='" + iconLink + "' taskId='" + taskID + "' itemType='" + itemType + "' downLink='" + downLink + "' playStatus='" + playStatus + "' windowsNum='" + win + "'systemSound='" + systemSound + "' ><span class='programListSpan' markID=" + markID + " itemType=" + itemType +" style='' onclick=\"showCtrlBar('ctrlBar" + markID + "','" + taskID + "','" + itemType + "',this);\" ><p class='programFileName' style='width:90%;'>" + moreInfo2[0] + "</p><p class='timeLong' style='width:90%;font-size:14px;color:#fff;margin-top:5px;'>" + infoProgram + "</p></span>" + columnInfoImage + "</div></div><div class='left1' taskid='" + taskID + "' style='display:none;'></div>";
                ...
                if (currentTask <= 0) {
                    currentTask = 1;
                    $(".programList").find(".left").eq(0).find(".programListSpan").click()
		...
		data:{
			...
			if($(data).find("item").length>0){
				...
				for(var i=0;i<dataStr.length;i++){
                    ...
                    var inImage = "<div class='startNow'><img  src='images/liebiao/bofang.png' class='playNowprogram' onclick=\"startNow(this,'" + taskID +"','"+1+"','"+$(ele).parent().parent().parent().attr('taskid')+"');\"/><img class='moreProgramInfo' src='images/liebiao/gengduo.png' /></div>";
                    var infoProgram = "<span class='infoProgram'>"+$(ele).parent().prev().find(".infoProgram").html()+"</span>";
                   
					content2 += "<div class='left folderList' taskid='" + taskID + "' columId=" + $(this).find("markID").text().split("-")[2]+" style='display:none;'>" + imagesrc + "<div class='leftList' iconLink='" + iconLink + "' taskId='" + taskID + "' itemType='" + tasktype + "' downLink='" + downLink + "' playStatus='" + playStatus + "' windowsNum='" + win + "'systemSound='" + systemSound + "' ><span class='programListSpan' markID=" + markID + " itemType=" + tasktype +" style='' onclick=\"showCtrlBar('ctrlBar" + markID + "','" + taskID + "','" + tasktype + "',this);\" ><p class='programFileName' style='width:90%;'>" +fileName+ "</p><p class='timeLong' style='width:90%;font-size:14px;color:#fff;margin-top:5px;'>" + infoProgram + "</p></span>" + inImage + "</div></div>" + programListInfo + "<div class='left1' taskid='" + taskID + "' style='display:none;'></div></div>";
				}	
...
function showCtrlBar(barID, taskID, itemtype, thisList) {
    ...
    else if (itemtype == 8 || itemtype == 1008)//office文档
    {
        ...
        $(".picture a").parent().remove();
        $(".picture ul").append(str);
    }else if(itemtype==12&&allName.split("-")[1].indexOf(":")>=0){
        sendUdpMsg();
        $(".udp").show();
        $(".musicOrVideo").css("display", "none");
        $(".ppt").css("display", "none");
...
function checkFileType(itemtype, downLink) {
    if (itemtype == 12 || itemtype == 1012 || itemtype == 0 || itemtype == 1000 || itemtype == 11 || itemtype == 1011) {
        ...
        } else if (downLink.indexOf(".wav") >= 0 || downLink.indexOf(".aif") >= 0 || downLink.indexOf(".mp3") >= 0 || downLink.indexOf(".wma") >= 0 || downLink.indexOf(".cda") >= 0 || downLink.indexOf(".au") >= 0 || downLink.indexOf(".midi") >= 0 || downLink.indexOf(".aac") >= 0 || downLink.indexOf(".ape") >= 0 || downLink.indexOf(".ogg") >= 0|| downLink.indexOf(".flac") >= 0) {
...
function changeStyle() {
    ...
    $(".moreProgramInfo").click(function () {
        if($(".folderList").length<=0){
            getFileFolder(this,$(this).parent().parent().attr("downlink"),$(this).parent().parent().attr("itemtype"));		
       }
       if ($(this).parent().parent().attr("itemtype").indexOf("14")>=0|| $(this).parent().parent().attr("itemtype").indexOf("1014")>=0) {
            for (var i = 0; i < $(".left").length; i++) {
                if ($(".left").eq(i).attr("columid") == $(this).parent().parent().parent().attr("taskid")) {
                    if ($(".left").eq(i).find(".leftList").attr("itemtype").indexOf("14")<0&& $(".left").eq(i).find(".leftList").attr("itemtype").indexOf("1014")<0) {
                        if ($(".left").eq(i).css("display") == "none") {
                            ...
                            $(this).attr("src", "images/contentTab/gengduoex1.png");
                        } else {
                            $(".left").eq(i).slideUp("slow");
                            $(this).attr("src", "images/contentTab/gengduoex.png");
                        }
                    }
                }
            }
       }else{
            if($(this).parent().parent().attr("downlink").slice($(this).parent().parent().attr("downlink").length-1).indexOf("\\")>=0){
                if($(".folderList").length<=0){
					$(this).attr("src", "images/contentTab/gengduoex1.png");
					setTimeout(function(){
...
function startNow(thisItem,taskId,folder,parentId) {
    ...
    var winNum=parseInt($("#top").attr("nowwindow"));
    var itemType=$(thisItem).parent().prev().attr("itemtype").split("-")[1].toLowerCase();
...
function docmd(cmdtype, cmdData) {
    ...
    if (!isNaN(cmdtype)) {//执行显示端命令
        ...
        var wpsendclientmsg="";
        if(cmdtype=="openSound"){
            wpsendclientmsg=(winNum*10000+3089)+"_0";
        }else if(cmdtype=="closeSound"){
            wpsendclientmsg=(winNum*10000+3088)+"_0";
        }
        $.ajax({
            data: { wpsendclientmsg:wpsendclientmsg },
            url: sendcmdurl,
            dataType: 'html',
            type: 'GET',
            timeout: 15000,		//超时时间
    ...
    if (cmdtype == "before") {
        var ele = $(".current").parent().prev().prev().prev();
        if(ele.length!=0){
            ele.find(".leftList").addClass("current").find(".playNowprogram").click();
            ele.next().find(".leftList").removeClass('current');
        }else{
            hideLoading();
        }
...
function browserRedirect() {
    ...
    if (bIsIphoneOs || bIsWM || bIsAndroid || bIsWM) {
        window.location.href = window.location.href.split("newpctrl")[0] + "newmctrl"+window.location.href.split("newpctrl")[1];
    } else {
...
function sendUdpMsg(){
    ...
    var sendIp=newurl.split(":")[1].split("//")[1];
    if(newurl.indexOf("192.168.1.108")>=0){
        str='<img class="before image" src="udp/udpSwiper.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'replay\',\'3456\',\''+sendIp+'\')"><img class="before image" src="udp/udpReturn.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'idle\',\'3456\',\''+sendIp+'\')" /><img class="before image" src="udp/udpVideo1.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'playvideo1\',\'3456\',\''+sendIp+'\')" /><img class="before image" src="udp/udpVideo2.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'playvideo2\',\'3456\',\''+sendIp+'\')" />'
    }else if(newurl.indexOf("192.168.1.111")>=0){
        //str='<img class="before image" src="udp/udpPlay.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'pause\',\'8000\',\''+sendIp+'\')"><img class="before image" src="udp/udpPause.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'play\',\'8000\',\''+sendIp+'\')" /><img class="before image" src="udp/udpReplay.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'replay\',\'8000\',\''+sendIp+'\')" /><img class="before image" src="udp/udpAwaitVideo.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'v1\',\'8000\',\''+sendIp+'\')" /><img class="before image" src="udp/udpvideo.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'v2\',\'8000\',\''+sendIp+'\')" />'
		str='<img class="before image" src="udp/udpPlay.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'pause\',\'8000\',\''+sendIp+'\')"><img class="before image" src="udp/udpPause.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'play\',\'8000\',\''+sendIp+'\')" /><img class="before image" src="udp/udpReplay.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'replay\',\'8000\',\''+sendIp+'\')" /><img class="before image" src="udp/udpAwaitVideo.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'v1\',\'8000\',\''+sendIp+'\')" /><img class="before image" src="udp/udpvideo.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'v2\',\'8000\',\''+sendIp+'\')" /><img class="before image" src="udp/udpvideo.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'v3\',\'8000\',\''+sendIp+'\')" />'
    }else if(newurl.indexOf("192.168.1.112")>=0){
        str='<img class="before image" src="udp/udpPlay.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'pause\',\'8000\',\''+sendIp+'\')"><img class="before image" src="udp/udpPause.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'play\',\'8000\',\''+sendIp+'\')" /><img class="before image" src="udp/udpReplay.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'replay\',\'8000\',\''+sendIp+'\')" />'
    }else if(newurl.indexOf("192.168.1.128")>=0){
        str='<img class="before image" src="udp/udpPrev.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'prev\',\'6699\',\''+sendIp+'\')"><img class="before image" src="udp/udpNext.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'next\',\'6699\',\''+sendIp+'\')" /><img class="before image" src="udp/udpReturn.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'idle\',\'6699\',\''+sendIp+'\')" /><img class="before image" src="udp/udpAwaitVideo.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'playvideo\',\'6699\',\''+sendIp+'\')" />'
    }else if(newurl.indexOf("192.168.1.129")>=0){
        str='<img class="before image" src="udp/udpPrev.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'prev\',\'1999\',\''+sendIp+'\')"><img class="before image" src="udp/udpNext.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'next\',\'1999\',\''+sendIp+'\')" /><img class="before image" src="udp/udpReturn.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'idle\',\'1999\',\''+sendIp+'\')" />'
    }else if(newurl.indexOf("192.168.1.130")>=0){
        str='<img class="before image" src="udp/udpPrev.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'prev\',\'2999\',\''+sendIp+'\')"><img class="before image" src="udp/udpNext.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'next\',\'2999\',\''+sendIp+'\')" /><img class="before image" src="udp/udpReturn.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'idle\',\'2999\',\''+sendIp+'\')" />'
    }else if(newurl.indexOf("192.168.1.131")>=0){
        str='<img class="before image" src="udp/udpPrev.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'prev\',\'3999\',\''+sendIp+'\')"><img class="before image" src="udp/udpNext.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'next\',\'3999\',\''+sendIp+'\')" /><img class="before image" src="udp/udpReturn.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'idle\',\'3999\',\''+sendIp+'\')" />'
    }else if(newurl.indexOf("192.168.1.132")>=0){
        str='<img class="before image" src="udp/udpPrev.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'prev\',\'4999\',\''+sendIp+'\')"><img class="before image" src="udp/udpNext.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'next\',\'4999\',\''+sendIp+'\')" /><img class="before image" src="udp/udpReturn.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'idle\',\'4999\',\''+sendIp+'\')" />'
    }else if(newurl.indexOf("192.168.1.133")>=0){
        str='<img class="before image" src="udp/udpPrev.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'prev\',\'5999\',\''+sendIp+'\')"><img class="before image" src="udp/udpNext.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'next\',\'5999\',\''+sendIp+'\')" /><img class="before image" src="udp/udpReturn.png" style="margin-right:3%;" onclick="sendCommonOrder(\''+hostIp+'\',\'udp\',\'idle\',\'5999\',\''+sendIp+'\')" />'
    }
    $(".udp").html(str);
}
