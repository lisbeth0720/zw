...
$(function () {
	...
	$(".clickMore img").click(function () {
		...
		if (parseInt($(".topRight").css("right")) != -parseInt($(".top").width() * 0.35) && parseInt($(".topRight").css("right")) != -35) {
			if (parseInt($(".topRight").css("right")) == -100) {
				$(".topRight").show();
				$(".topMain").animate({ "width": "100%" });
...
window.onload = function () {
	...
	$(".cc").click(function () {
		//debugger;
		var clientBg = $(".topLeft .clientList").css("background");
		var clientColor = $(".cc").css("color");
		if (clientColor == "") {
			clientColor = "#000";
		}
		...
		$(".topLeft .clientList").css("background", clientBg);
		$(".cc").css({ "background": clientBg, "color": clientColor });
	...
	if ($(".commonP").css("display") == "none") {
		$(".topMain").css("height", $(".top").height() - $(".topLeft").height())
	}
...
function getData(URL, num) {
	...
	$.ajax({
		...
		success: function (xml) {	//if($(xml).text()!)
			...
			if ($(xml).find("item").length <= 0) {
				...
				$(xml).find("item").each(function (i) {
					...
					if ($(this).find("itemType").text() == "14" || $(this).find("taskName").text().indexOf("**") >= 0 || $(this).find("itemType").text() == "1014") {
...
//根据节目项的类型显示相应的类型（在没有素材名称时，显示素材的类型名）
...
function changeStyle() {
	...
	if ($(".controlBottomTab li span").css("display") == "none") {
	} else {
	...
	for (let i = 0; i < document.getElementById("Screen").getElementsByClassName("left").length; i++) {
		...
		document.getElementsByClassName("leftList")[i].ontouchstart = function (evt) {
			...
			document.getElementsByClassName("leftList")[i].ontouchend = function (evt) {
				...
				} else if (parseInt(oDiv.parentNode.parentNode.style.left) <= -20) {
					$("#upload_File").click();
					showPrint("1")
					console.log("打印");
...
function MobileGoHis() {	
	$(".topRight").animate({ "right": "-100%" });
	$(".topMain").animate({ "width": "100%" });
	setTimeout(function () {
		$(".topRight").hide();
	}, 500)
