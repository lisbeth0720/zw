...
{
    public partial class Sys_right : System.Web.UI.Page
    {
            ...
            if (mmodel.IsOnLine == 1)
            {
                if (!string.IsNullOrEmpty(Request["id"]) && !string.IsNullOrEmpty(Request["idtype"]))
                {
                    ...
                    if (cmService.IsShouQuan(mmodel, 0, CommonRight.RIGHT_C_Auth) || mmodel.Userid == authid)
                    {
                        ...
                        DataTable aTable = aService.GetAllRightByAuthID(mmodel.Companyid, authid, 0, idtype);
                        ...
                        if (idtype == 0)//0用户，1部门
                        {
                            ...
                            DataTable pTable = aService.GetAllRightByAuthID(manager.Companyid, Convert.ToInt32(manager.DeptID), 0, 1);
